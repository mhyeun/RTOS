 ...
 ****************************************************************************/
 ...
 /*
 * Names:
 *   1.  uWaterloo User ID: 20765871 jehtiouf@uwaterloo.ca
 *   2.  uWaterloo User ID: 20774652 jhyeun@uwaterloo.ca
 */
...
*               to main.c or bst.c.
*****************************************************************
*  1. Adding curly braces for if/else if/else statements
*  2. Change incrementing from x = x + 1 to x += 1 for conciseness
*  3.	Tabbing inside curly braces for clarity(if, else, while, etc.)
*  4.	Removing unnecessary whitespace for consistency
*  5.	Improved commenting (changed case 3 to 2 for chronological accuracy)
****************************************************************/
...
*****************************************************************
*  1. bst_delete, line 207: replace bitwise AND (&) to operator AND (&&) (Found using step one line)
*  2. bst_delete, line 254: changed p_parentNode->right to p_parentNode->left, vice versa for line 256 (Found with watch window)
*  3. bst_erase, line 308: change += 1 to -= 1 when decrementing tree size (Comment said decrement)
*  4. bst_delete, line 218: changed ->left to -> right since we are looking for max

Also: bst_delete, line 187: add * to parameters in delete_bst to make them pointers (Type error, -> can only be used with pointers)
...
void bst_delete(bsn_t *p_currNode, bsn_t *p_parentNode)
{
	...
	// case 2: two children, pick one to replace with
	...
	// case 3: one child, replace with child, delete child
	else
	{
		if(p_currNode->left != NULL) //child is to the left
		{
			...
			if(p_parentNode->left == p_currNode)
			{
				p_parentNode->left = p_currNode->left; // setting parent's left pointer to target's left child
			}
			else
			{
				p_parentNode->right = p_currNode->left; // setting parent's right pointer to target's left child
			}
		}
		else // child is to the right
		{
			...
			if(p_parentNode->left == p_currNode)
			{
				p_parentNode->left = p_currNode->right; // setting parent's left pointer to target's right child
			}
			else
			{
				p_parentNode->right = p_currNode->right; // setting parent's right pointer to target's right child
			}
		}
	}
